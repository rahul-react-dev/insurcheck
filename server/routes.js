...
router.get('/super-admin/system-config', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    ...
    // Group configurations by category for the frontend
    ...
    console.log(`✅ Found ${configs.length} system configurations in ${Object.keys(configsByCategory).length} categories`);
...
router.get('/super-admin/system-config/:key', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    const configKey = req.params.key;
    console.log(`📋 Fetching system configuration: ${configKey}`);
    ...
    if (config.length === 0) {
      return res.status(404).json({
        ...
        message: `System configuration with key '${configKey}' not found`
      });
    }

    console.log(`✅ Found system configuration: ${configKey}`);
    res.json({ configuration: config[0] });
  } catch (error) {
...
router.post('/super-admin/system-config', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    ...
    console.log(`📝 Creating system configuration: ${key}`);
    ...
    if (existingConfig.length > 0) {
      return res.status(409).json({
        ...
        message: `System configuration with key '${key}' already exists`
      });
    }
    ...
    console.log(`✅ Created system configuration: ${key}`);
    res.status(201).json({ 
...
router.put('/super-admin/system-config/:key', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    ...
    console.log(`📝 Updating system configuration: ${configKey}`);
    ...
    console.log(`✅ Updated system configuration: ${configKey}`);
    res.json({ 
...
router.delete('/super-admin/system-config/:key', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    const configKey = req.params.key;
    console.log(`🗑️ Deleting system configuration: ${configKey}`);
    ...
    console.log(`✅ Deleted system configuration: ${configKey}`);
    res.json({ 
...
router.put('/super-admin/system-config', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    const { configurations } = req.body;
    console.log(`📝 Bulk updating ${configurations.length} system configurations`);

    if (!Array.isArray(configurations) || configurations.length === 0) {
      return res.status(400).json({
        error: 'Invalid input',
        message: 'Configurations array is required and must not be empty'
      });
    }
    ...
    for (const config of configurations) {
      try {
        const { key, value, description, category, isActive } = config;
        
        if (!key) {
          errors.push({ key: 'unknown', error: 'Key is required' });
          continue;
        }
        ...
        updatedConfigs.push(updatedConfig[0]);
      } catch (error) {
        errors.push({ key: config.key || 'unknown', error: error.message });
      }
    }

    console.log(`✅ Bulk updated ${updatedConfigs.length} configurations with ${errors.length} errors`);
    res.json({ 
      message: `Bulk update completed: ${updatedConfigs.length} updated, ${errors.length} errors`,
    ...
    res.status(500).json({ 
      error: 'Failed to bulk update system configurations',
      message: error.message 
    });
  }
});
