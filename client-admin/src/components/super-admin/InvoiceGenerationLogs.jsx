        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
          <div>
            <Input
              ...
              onChange={(e) => setLocalFilters(prev => ({ ...prev, tenantName: e.target.value }))}
      ...
      <div className="space-y-4">
        {logs.length === 0 ? (
          <Card className="p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No invoice logs found</h3>
            <p className="text-gray-500">Invoice generation logs will appear here</p>
          </Card>
        ) : (
          logs.map((log) => (
              ...
              <div className="p-6">
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-3 mb-2">
                        <h3 className="text-lg font-semibold text-gray-900">{log.invoiceId}</h3>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                        <div>
                          ...
                          <span className="ml-1 font-medium">{log.tenantName}</span>
                        </div>
                        <div>
                          <span className="text-gray-500">Amount:</span>
                          <span className="ml-1 font-medium">{formatCurrency(log.amount)}</span>
                        </div>
                        <div>
                          <span className="text-gray-500">Generated:</span>
                          <span className="ml-1 font-medium">{formatDateTime(log.generationDate)}</span>
                        </div>
                        <div>
                          <span className="text-gray-500">Method:</span>
                          <span className="ml-1 font-medium capitalize">{log.generationType}</span>
                        </div>
                      </div>
                      ...
                      {log.sentDate && (
                        <div className="mt-2 text-sm">
                          ...
                          <span className="ml-1 font-medium">{formatDateTime(log.sentDate)}</span>
                          <span className="text-gray-500 ml-4">To:</span>
                          <span className="ml-1 font-medium">{log.sentToEmail}</span>
                        </div>
                      )}
                      ...
                      {log.notes && (
                        <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                          <div className="flex items-center">
                            ...
                            <span className="text-sm text-blue-700 font-medium">Notes:</span>
                          </div>
                          <p className="text-sm text-blue-600 mt-1">{log.notes}</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 flex-shrink-0">
                    {log.status === 'failed' && (
                      <Button
                        onClick={() => onRetryGeneration && onRetryGeneration(log.id)}
