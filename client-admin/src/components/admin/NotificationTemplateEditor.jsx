...
export function NotificationTemplateEditor({ template, onClose, onSave }) {
  ...
  const templateTypeConfig = {
    compliance_result: {
      ...
      variables: [
        '{{userName}}', '{{organizationName}}', '{{documentName}}', 
        '{{complianceStatus}}', '{{rulesChecked}}', '{{rulesPassed}}', 
        '{{rulesFailed}}', '{{timestamp}}', '{{detailsUrl}}'
      ]
    },
    audit_log: {
      icon: Activity,
      ...
      variables: [
        '{{userName}}', '{{organizationName}}', '{{action}}', 
        '{{resource}}', '{{timestamp}}', '{{ipAddress}}', 
        '{{details}}', '{{logUrl}}'
      ]
    },
    user_notification: {
      ...
      variables: [
        '{{userName}}', '{{organizationName}}', '{{notificationType}}', 
        '{{message}}', '{{timestamp}}', '{{actionRequired}}', '{{actionUrl}}'
      ]
    },
    system_alert: {
      icon: AlertTriangle,
      ...
      variables: [
        '{{organizationName}}', '{{alertType}}', '{{severity}}', 
        '{{message}}', '{{timestamp}}', '{{affectedServices}}', '{{statusUrl}}'
      ]
    }
  };
  ...
  const applyTemplate = (type) => {
    const templates = {
      compliance_result: {
        subject: '[{{organizationName}}] Compliance Check Results for {{documentName}}',
...
Status Updates: {{statusUrl}}`
    ...
    toast({
      ...
      description: 'The template has been applied. You can now customize it further.',
    });
  };

  const currentTypeConfig = templateTypeConfig[formData.templateType] || templateTypeConfig.user_notification;
                ...
                <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-sm text-blue-800">
                      <strong>Tip:</strong> Place your cursor in the text area and click a variable to insert it at that position.
                    </div>
                  </div>
                </div>
              </CardContent>
                ...
                <div className="border rounded-lg p-4 bg-white">
                  <div className="space-y-4">
                    <div className="border-b pb-3">
                      <h4 className="font-medium text-sm text-gray-600">Subject</h4>
                      <p className="mt-1 font-semibold">{preview.subject}</p>
                    </div>

                    {preview.header && (
                      <div className="border-b pb-3">
                        <h4 className="font-medium text-sm text-gray-600">Header</h4>
                        <div className="mt-1 whitespace-pre-line">{preview.header}</div>
                      </div>
                    )}

                    <div className="border-b pb-3">
                      <h4 className="font-medium text-sm text-gray-600">Body</h4>
                      <div className="mt-1 whitespace-pre-line">{preview.body}</div>
                    </div>

                    {preview.footer && (
                      <div>
                        <h4 className="font-medium text-sm text-gray-600">Footer</h4>
                        <div className="mt-1 whitespace-pre-line text-gray-600">{preview.footer}</div>
                      </div>
                    )}
                  </div>
                </div>
